...
* `"./ontime-db:/external/db/"` and `"./ontime-styles:/external/styles/"` expose the internal container directories.
...
These three paths can be bound to individual host directories as in the sample docker-compose file, or you can bind a single host directory to /external/ for Ontime to store all its files in.
